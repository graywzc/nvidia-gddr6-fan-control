...
function renderHoncho(d){let h=d.honcho_health||{};let el=document.getElementById('honchoInfo');if(!el)return;let api=h.api||'unknown';let apiCls=api==='up'?'good':'critical';let embedding=h.embedding||'unknown';let embeddingCls=embedding==='up'?'good':'critical';let deriver=h.deriver_status||'unknown';let deriverCls=(deriver==='active'||deriver==='caught up')?'good':(deriver==='idle'?'hot':'critical');let rows='';rows+=infoRow('API',`<span class=\"${apiCls}\">${esc(api)}</span>`);let embeddingInfo=`<span class=\"${embeddingCls}\">${esc(embedding)}</span>`;if(h.embedding_model)embeddingInfo+=' <span class=\"label\">'+esc(h.embedding_model)+'</span>';if(h.embedding_dims)embeddingInfo+=' <span class=\"label\">'+Number(h.embedding_dims).toLocaleString()+'d</span>';rows+=infoRow('Embedding',embeddingInfo);let deriverInfo=`<span class=\"${deriverCls}\">${esc(deriver)}</span>`;if(h.deriver_last)deriverInfo+=' <span class=\"label\">'+esc(h.deriver_last)+'</span>';if(h.pending_msgs!=null&&h.pending_msgs>0)deriverInfo+=` <span class=\"hot\">${h.pending_msgs} msgs behind</span>`;rows+=infoRow('Deriver',deriverInfo);if(h.batch_tokens!=null&&h.batch_max!=null){let pct=Math.min(100,Math.round(100*h.batch_tokens/h.batch_max));rows+=`<div class=\"row\"><span class=\"label\">Batch</span><span class=\"value\" style=\"display:flex;align-items:center;gap:8px\"><div class=\"bar\" style=\"flex:1;min-width:60px\"><div class=\"fill\" style=\"width:${pct}%\"></div></div><span style=\"font-size:11px\">${h.batch_tokens}/${h.batch_max} tokens</span></span></div>`}if(h.messages!=null)rows+=infoRow('Messages',Number(h.messages).toLocaleString()+(h.last_message_at?' <span class=\"label\">'+esc(h.last_message_at)+'</span>':''));if(h.documents!=null)rows+=infoRow('Observations',Number(h.documents).toLocaleString()+(h.last_document_at?' <span class=\"label\">'+esc(h.last_document_at)+'</span>':''));if(h.queue_pending!=null)rows+=infoRow('Queue',Number(h.queue_pending).toLocaleString());el.innerHTML=rows||'<div class=\"row\"><span class=\"label\">No Honcho data yet</span></div>'}
