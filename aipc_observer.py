...
function flagCompareRows(vars,rows){if(!rows||!rows.length)return '<div class="row"><span class="label">No command flags found</span></div>';let cols=`minmax(150px,.8fr) minmax(260px,1.3fr) repeat(${vars.length}, minmax(180px,1fr))`;let html=`<div class="flag-compare" style="grid-template-columns:${cols}"><div class="flag-cell head">Flag</div><div class="flag-cell head">Definition</div>`+vars.map(v=>`<div class="flag-cell head">${esc(v.variant)}</div>`).join('');html+=rows.map(r=>{let vals=vars.map(v=>Object.prototype.hasOwnProperty.call(r.values||{},v.variant)?r.values[v.variant]:undefined);let present=vals.filter(v=>v!==undefined).map(v=>v===null?'':String(v));let changed=new Set(present).size>1||present.length!==vals.length;let desc=r.known?esc(r.description||'listed in --help without description'):'<span class="hot">not found in --help</span>';let aliases=(r.aliases||[]).filter(a=>a!==r.flag).join(', ');let title=aliases?` title="aliases: ${esc(aliases)}"`:'';return `<div class="flag-cell mono"${title}>${esc(r.flag)}</div><div class="flag-cell flag-desc">${desc}</div>`+vals.map(v=>`<div class="flag-cell mono ${changed?'changed':'same'}">${flagValue(v)}</div>`).join('')}).join('');return html+'</div>'}
function renderCompareModal(data){let vars=data.variants||[];let help=data.help||{};let helpNote=help.error?`<div class="row"><span class="critical">help: ${esc(help.error)}</span></div>`:(help.source?`<div class="row"><span class="label">Help source</span><span class="value mono">${esc(help.source)}${help.warning?' · '+esc(help.warning):''}</span></div>`:'<div class="row"><span class="label">Help source</span><span class="value">not available</span></div>');document.getElementById('compareModalTitle').textContent=`Command compare (${vars.length})`;document.getElementById('compareModalBody').innerHTML=helpNote+flagCompareRows(vars,data.flag_matrix||[])+det('detCompareRaw',false,'raw compose details',`<div class="compare-grid">${vars.map(compareCard).join('')}</div>`);document.getElementById('compareModal').classList.add('open')}
