...
.flag-guide{display:grid;grid-template-columns:minmax(92px,.7fr) minmax(70px,.8fr) minmax(180px,2fr);gap:8px;align-items:start;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px}.flag-guide.head{position:sticky;top:0;background:var(--surface);color:var(--dim);font-size:11px;text-transform:uppercase;font-weight:700}.flag-help{color:var(--text);line-height:1.35}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
</style>
</head>
<body>
...
function renderCommandGuide(mi){let guide=mi.command_guide||[];if(!guide.length)return '';let help=mi.help||{};let summary=help.error?`flag guide (${guide.length}, help failed)`:help.source?`flag guide (${guide.length}, from ${esc(help.source)})`:`flag guide (${guide.length})`;let rows='<div class="flag-guide head"><span>Flag</span><span>Value</span><span>Help text</span></div>';rows+=guide.map(r=>{let desc=r.known?esc(r.description||'listed in --help without description'):'<span class="hot">not found in --help</span>';let aliases=(r.aliases||[]).filter(a=>a!==r.flag).join(', ');let title=aliases?` title="aliases: ${esc(aliases)}"`:'';return `<div class="flag-guide"><span class="mono"${title}>${esc(r.flag)}</span><span class="mono">${r.value==null?'<span class="label">switch</span>':esc(r.value)}</span><span class="flag-help">${desc}</span></div>`}).join('');if(help.error)rows+=`<div class="row"><span class="critical">${esc(help.error)}</span></div>`;return det('detFlagGuide',false,summary,`<div style="max-height:420px;overflow:auto">${rows}</div>`)}
